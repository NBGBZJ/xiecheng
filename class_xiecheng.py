...
class DoXC:
    ...
    def __init__(self, yearDate, DepartPort, ArrivePort, tag):
        ...
        self.post_headers = {
            'Host': 'exchdata.ctrip.com', 'Content-type': 'application/json',
    ...
    def get_data(self):
        ...
        try:
            ...
            # log_set(name='class_xiecheng', msg
            # ='[xiecheng]携程找到航班:%s,%s'%(info_list,self.yearDate))
            return info_list
        except:
                ...
                if xc_flight_no == fligh_name_feiba:    # flight
                    ...
                    if float(xc[1]) - float(feiba_price) > 6:  # 推送条件
                        ...
                        if self.tag in (0, '0'):  # new come
                            # save_xc_info(self.yearDate,
                            # self.DepartPort,self.ArrivePort,xc[2],
                            # str(float(feiba_price)),inVent,xc_id=xc[0])
                            ...
                            if not xc_id:   # never push
                                ...
                                # save_xc_info(YearMonthDate1=day,
                                # DepartPort=deptCd,ArrivePort=arrCd,
                                # Flight_No=xc[1],Flight_Price=float(xc[0]),inVent=int(feiba_info[(xc[1]).upper()][0]))
